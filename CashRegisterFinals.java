...
class Product {
    ...
    Product(String name, String size, double price, int quantity) {
        this.name = name;
        this.size = size;
        this.price = price;
    ...
    public String toString() {
        return name + " (" + size + ") | Price: P" + String.format("%.2f", price) + " | Qty: " + quantity + " | Total: P" + String.format("%.2f", getTotalPrice());
    }
}
...
public class CashRegisterFinals {
    ...
    // Predefined drink sizes and prices
    static final String[] SIZES = {"Small", "Medium", "Large", "Extra-Large"};
    static final double[] PRICES = {79.0, 89.0, 99.0, 119.0};
    ...
    public static void addProduct() {
        try {
            System.out.print("\nEnter Milktea flavor: ");
            String name = ashi.nextLine().trim();

            if (name.isEmpty()) {
                System.out.println("Error: Milktea flavor cannot be empty.\n");
                return;
            }
            ...
            for (int i = 0; i < SIZES.length; i++) {
                System.out.println((i + 1) + ". " + SIZES[i] + " - P" + String.format("%.2f", PRICES[i]));
            }

            int sizeChoice = getValidChoice("Choose size (1-" + SIZES.length + "): ", 1, SIZES.length) - 1;
            String selectedSize = SIZES[sizeChoice];
            double selectedPrice = PRICES[sizeChoice];

            int quantity = getValidInt("Enter Milktea quantity: ");

            cart.add(new Product(name, selectedSize, selectedPrice, quantity));
            System.out.println("\n " + selectedSize + " " + name + " Milktea added to cart!\n");

        } catch (Exception e) {
            System.out.println("Error: Failed to add Milktea. Please try again.\n");
        }
    }
    ...
    public static void updateProductQuantity() {
        ...
        try {
            displayCart();
            int index = getValidChoice("Enter Milktea number to update (1-" + cart.size() + "): ", 1, cart.size()) - 1;

            Product product = cart.get(index);
            System.out.println("Current quantity for '" + product.name + " (" + product.size + ")': " + product.quantity);
            ...
            System.out.println("Error: Failed to update Milktea quantity.\n");
        }
    }
    ...
    public static void removeProduct() {
        ...
        try {
            displayCart();
            int index = getValidChoice("Enter Milktea number to remove (1-" + cart.size() + "): ", 1, cart.size()) - 1;

            Product removedProduct = cart.remove(index);
            System.out.println("\n " + removedProduct.name + " (" + removedProduct.size + ")' removed from cart!\n");

        } catch (Exception e) {
            System.out.println("Error: Failed to remove Milktea.\n");
        }
    }
    ...
    public static void logTransaction(double total, double payment, double change) {
        try (FileWriter writer = new FileWriter("transactions.txt", true)) {
            ...
            for (Product p : cart) {
                writer.write("  - " + p.name + " (" + p.size + ") | Qty: " + p.quantity + " | Price: P" +
    ...
    public static void main(String[] args) {
        System.out.println("\n=======================================");
        System.out.println("    Welcome to A&R Milktea House!!");
        System.out.println("  Enhanced online transaction System");
        System.out.println("=======================================");
        ...
        while (true) {
            try {
                ...
                switch (choice) {
                    ...
                    case 3:
                        System.out.println("\n================================================");
                        System.out.println(" Thank you for ordering in A&R Milktea House!");
    ...
    public static void runCashRegister() {
        while (true) {
            try {
                ...
                System.out.println("1. Add Milktea to Cart");
                System.out.println("2. View Cart");
                System.out.println("3. Update Milktea Quantity");
                System.out.println("4. Remove Milktea from Cart");
